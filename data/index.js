...
const randomStory = (data, user_id) => {
    if(data === undefined) {
        data = {
...
const randomComment = (data, the_user, the_story) => {
    if (data === undefined) {
        data = {
            body: `${faker.name.firstName()} is one of the best programmers ever!!!`
        }
    }

    return {
        body: `${data.body}`,
        userId: Number(the_user),
        storyId: Number(the_story),
        createdAt: `${randomYear()}-${randomMonth()}-${randomDay()}`,
        updatedAt: new Date()
    };
};
...
const comments_array = [
    ...
    {
        body: `Hey guys my email is ${faker.internet.email()}.
        If you ever need a developer and are re-working a website here is my
        contact info.
